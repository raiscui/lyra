...
## [2026-03-15 06:48:00 UTC] 主题: 多相机轨迹系统最好拆成"轨迹 preset 枚举层"和"几何轨迹生成层"

### 发现来源
- 本轮把 `multi_trajectory` 的实现整理成给其他项目复用的文档时, 重新梳理了:
...
### 核心问题
- 很多项目会把"有哪些轨迹"和"轨迹数学怎么生成"混写在一个大函数里.
- 这样短期能跑, 但后面一旦要新增轨迹、调整输出组织、切训练/推理模式, 修改范围会迅速失控.

### 为什么重要
- 只要把这两层拆开:
  - preset 枚举层负责批量策略、目录组织、强度采样
  - 几何生成层负责轨迹公式、朝向更新、位姿输出
- 新增功能时就能做到局部修改, 不会把整条链路一起搅乱.

### 当前结论
- 这次文档化再次证明, 当前实现最可复用的部分不是某一条具体轨迹, 而是这个两层拆分结构.
- 对其他项目来说, 优先迁移这层结构, 往往比原样照抄某几段数学公式更有价值.
