...
## 2026-03-09 任务: 排查 static view 索引与 dataset_registry 对应关系

### 当前目标

- 核对 `static_view_indices_fixed=['5','0','1','2','3','4']` 与 `dataset_registry` 中 `sampling_buckets=[['0'],['1'],['2'],['3'],['4'],['5']]`、`start_view_idx=0` 是否存在错位.
- 区分这几个字段到底表示:
  - 视角 ID 顺序
  - 采样桶位置
  - 数据集内部偏移
- 只在证据充分后给出结论, 避免把候选假设说成根因.
...
- 现象:
  - 用户观察到 `static_view_indices_fixed` 的顺序是 `5,0,1,2,3,4`.
  - `dataset_registry` 里的 `sampling_buckets` 顺序是 `0,1,2,3,4,5`, `start_view_idx=0`.
- 当前假设:
  1. 这三者可能使用了不同语义, 因此表面顺序不同但不一定错.
  2. 也可能存在 "第 0 个 bucket 实际对应视角 5" 之类的偏移风险.
- 最小验证:
  - 查定义位置.
  - 查运行时如何把 `static_view_indices_fixed` 传给 dataloader / sampler.
  - 查 `start_view_idx` 是否只影响迭代起点, 还是参与真实 ID 映射.

### 状态

**进行中**: 正在定位相关代码与配置, 准备做映射核对.

### 2026-03-09 进展更新

- [x] 阶段1: 回读历史上下文与现有实验记录
- [x] 阶段2: 定位字段定义与消费路径
- [x] 阶段3: 做一致性判断并形成结论
- [ ] 阶段4: 留痕
...
- `static_view_indices_fixed` 在 fixed 模式下直接作为最终 view ID 列表使用.
- `sampling_buckets` 只在 `random_bucket` 模式下参与映射.
- `start_view_idx=0` 说明当前数据集 view 编号从 `0` 开始,不会把 `5` 自动重映射成别的桶位.
- 因此当前这组配置表面顺序不同,但语义并不冲突.

### 状态

**进行中**: 已拿到静态与动态证据, 正在整理成面向用户的判断与边界条件.
...
- 当前配置下, `static_view_indices_fixed=['5','0','1','2','3','4']` 与 registry 中 `sampling_buckets=[['0'],['1'],['2'],['3'],['4'],['5']]`、`start_view_idx=0` 不构成错位 bug.
- 原因是 fixed 模式直接消费 `static_view_indices_fixed`, bucket 顺序不会参与映射.
- 只有切回 `random_bucket` 或修改数据集 view 编号体系时, 才需要重新检查 bucket / start_view_idx.

### 状态

**已完成**: 已基于代码路径与最小动态验证确认对应关系, 可以向用户给出结论.
