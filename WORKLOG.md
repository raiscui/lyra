...
## [2026-03-14 17:10:00 UTC] 任务名称: 将 `gen3c_single_image_sdg.py` 的视频编码质量固定为 9

### 任务内容
- 按用户要求只调整 `gen3c_single_image_sdg.py`
- 把 mp4 保存质量从固定 `8` 改为固定 `9`
- 不改 `gen3c_dynamic_sdg.py`

### 完成过程
- 先确认脚本内有两处 `save_video(..., video_save_quality=8, ...)`
- 然后在文件顶部新增 `SDG_VIDEO_SAVE_QUALITY = 9`
- 再让 resume 补写路径和正常输出路径都复用这一个常量
- 最后用 `rg` 和 `python3 -m py_compile` 做最小验证
