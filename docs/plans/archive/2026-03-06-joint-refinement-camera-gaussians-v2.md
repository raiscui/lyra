# Joint Refinement Camera Gaussians V2 Implementation Plan (Archived Final Completed Version)

> **For Claude:** REQUIRED SUB-SKILL: Use `superpowers:executing-plans` to implement this plan task-by-task.

> **Archive Status (2026-03-07):** 这份文档对应的原始 `Task 1 ~ Task 13` 已全部完成,正文也已追平到 2026-03-07 的最新实现状态。本文档现作为归档副本保留,当前仍在推进的增强工作请继续查看 `docs/plans/2026-03-06-long-lrm-style-post-refinement.md`。
...
- **当前测试状态:** `PYTHONPATH="$(pwd)" pytest -q tests/refinement_v2` 已到 `82 passed`
- **当前主线能力:**
  ...
  - pruning
  - patch supervision
  - full-view root mode(`--scene-stem` / `--view-ids` / `--pose-root` / `--intrinsics-root` / `--rgb-root` / `--reference-root`)
  - selective SR
  - `L_sampling_smooth`
  - before/after 视频导出
- **原计划闭环后的增量能力:**
  ...
  - direct file inputs v1:
    ...
    - `--intrinsics-path`
    - `--rgb-path`
  - direct-input `cam_view` 契约修复
  - `Stage 2B` warm-start resume 数量不一致恢复修复
- **历史上已完成真实验证的关键产物(当前工作区不保证仍全部保留):**
...
- 已新增:
  ...
  - `--rgb-path`
- 当前作用:
  - `build_scene_bundle(...)` 已经既能走 provider 模式
  - 也能直接从本地 `pose / intrinsics / rgb` 文件组装 `SceneBundle`
- 补充说明:
  - 这部分在本文档早期版本里曾被写成“未来方向”
  - 但到归档时已经实际落地,不再属于未实现项

### 4. 归档时仍属于下一阶段的新工作

这些不属于本计划的“未完成项”,而是归档后继续推进的增强方向:
...
## Archive Addendum (2026-03-07)

这部分补的是原计划完成之后, 又在同一条 `refinement_v2` 主线上继续落地并验证过的最新状态.

### 1. 真实 SR reference 已经打通

- `FlashVSR-Pro` 已在 48G 主机上真实跑通 `full mode`
- 真实参考视频产物:
...
### 2. `Stage 3SR` 已做过真实 smoke test 与修复后回归

- 真实 `SR reference` smoke test 输出:
  - `outputs/refine_v2/view3_stage3sr_real_sr_baseline_v1`
- 后续定位出 direct-input `cam_view` 契约问题后,已完成修复与回归:
  - `outputs/refine_v2/view3_stage3sr_real_sr_baseline_v1_fixed_cam_view`
- 修复后关键指标:
...
### 4. `Stage 2B` 当前正式推荐主线已定

- 基于 native baseline 的真实 `Stage 2B` 输出:
  - `outputs/refine_v2/view3_stage2b_from_native_reference_v1_fixed_cam_view_20260307_1248`
- 关键结果:
...
- 当前归档结论:
  - `native baseline -> Stage 2B` 是正式推荐主线
  - `SR -> Stage 2B` 可保留为锐度优先的可选分支

### 5. 关于“产物目录是否还在”的说明

- 本文档里较早记录的 `outputs/refine_v2/view5_*` 等路径,代表的是“历史上确实完成过的验证”
- 但这些目录不保证在后续工作区里一直保留
- 因此判断本计划是否完成时,应以:
  - 当前代码
  - 当前测试
  - 工作记录
  作为主证据,而不是只看旧输出目录是否仍在
